...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        def range_check(pos):
            return self.min_range <= pos <= self.max_range
        nearest_bee_place, scan_pos = self.place, 0

        while nearest_bee_place != beehive:
            if nearest_bee_place.bees and range_check(scan_pos):
                return rANTdom_else_none(nearest_bee_place.bees)
...
# END Problem EC
    ...
    def action(self, gamestate):
        ...
        def queens_bravery():
            ...
            while p:
                if p.ant:
                    if not hasattr(p.ant, 'bravery'):
                        p.ant.bravery = True
                        if hasattr(p.ant, 'damage'):
                            p.ant.damage *= 2
                p = p.exit
...
def make_slow(action, bee):
    ...
    def slow_action(gamestate):
        slow_action.slow = True
        if gamestate.time % 2 == 0:
            action[0](gamestate)  

            # I know you are pissed off by the selector.
            # But you have already been here,
            # why not check the apply_status function?
    
    slow_action.name = 'slow'     # debug
    return slow_action
...
def make_scare(action, bee):
    ...
    def scared_action(gamestate):
        #print('DEBUG: enter scared action')
        bee.is_backward = True
        action[0](gamestate)
        bee.is_backward = False
    
    scared_action.name = 'scare'    # debug
...
def apply_status(status, bee, length):
    ...
    def new_status(gamestate):
        """A decorator, which is the element of the status stack.

        This slow status is really weird.
        ...
        if new_status.duration:
            ...
            print('DEBUG: ' + new_status.action.name + ' turn left: ' + str(new_status.duration))
            new_status.action(gamestate) 
        else:
            """This legacy queue version can release memory, as in this scnario no later debuff will end before early one.
            bee.action = new_status.next[0]
            print('DEBUG: ' + new_status.action.name + ' is over and enter ' + str(bee.action.name))
    ...
    new_status.duration = length    # duration counter
    """legacy of regarding this as a queue
    t = bee.action                  # queue helper
    if hasattr(t, 'duration'):      # the bee has status in queue
        # append this status to the queue
        while not t.is_tail:
            t = t.next[0]
        new_status.next = [t.next[0]]
        t.next[0] = new_status
        new_status.is_tail, t.is_tail = True, False
        new_status.action = status(new_status.next, bee)
    else:
        new_status.is_tail = True
        new_status.next = [bee.action]
        new_status.action = status(new_status.next, bee)
    ...
    new_status.name = new_status.action.name    # debug
    # END Problem Optional 4
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        # BEGIN Problem Optional 4
        if target:
            if not hasattr(target,'has_been_scared'):
