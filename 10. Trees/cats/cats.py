...
def about(topic):
    ...
    def topic_checker(paragraph):
        paragraph = p_to_words(paragraph)
        for w1 in topic:
            for w2 in paragraph:
                if w1 == w2:
                    return True
...
def accuracy(typed, reference):
    ...
    # BEGIN PROBLEM 3
    right_words_count = 0
    compare_times = min(len(typed_words), len(reference_words))
    ...
    if typed_words_len:
        return right_words_count / typed_words_len * 100
    else:
        return 0.0
    # END PROBLEM 3
...
def shifty_shifts(start, goal, limit):
    ...
    # BEGIN PROBLEM 6
    """
    def helper(start, goal, errors = 0):
        if start and goal:
            if errors > limit:
                return errors
            elif start[0] == goal[0]:
                return helper(start[1:], goal[1:], errors)
            else:
                return helper(start[1:], goal[1:], errors + 1)
        else:
            return errors + (len(goal) or len(start))
...
def pawssible_patches(start, goal, limit):
    ...
    if limit < 0:  # Fill in the condition
        ...
        if start[0] != goal[0]:
            ...
            add_diff = remove_diff = substitute_diff = pawssible_patches(
...
def report_progress(typed, prompt, user_id, send):
    ...
    for i in range(len(typed)):
        if typed[i] == prompt[i]:
            ...
            break
    progress = right_words_count / len(prompt)
    package = {'id': user_id, 'progress': progress}
    send(package)
    return progress
    # END PROBLEM 8
...
def time_per_word(times_per_player, words):
    ...
    for i in range(len(times_per_player)):
        all_times += [[times_per_player[i][t] - times_per_player[i][t-1]
                       for t in range(1, len(times_per_player[i]))]]
...
def fastest_words(game):
    ...
    for w in word_indices:
        fastest_time, fastest_player = 100, 0
        for p in player_indices:
            if fastest_time > time(game, p, w):
                fastest_time, fastest_player = time(game, p, w), p
        words[fastest_player] += [word_at(game, w)]
