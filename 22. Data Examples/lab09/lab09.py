...
def subseqs(s):
    ...
    if s == []:
        ...
        all_subseqs = insert_into_all(s[0], subseqs(s[1:])) + subseqs(s[1:])
        return all_subseqs
...
def num_trees(n):
    ...
    *   *        *       *
    ...
    if n < 3:
        return 1
    return sum([num_trees(part_sum) * num_trees(n-part_sum) for part_sum in range(1, n)])
    ...
    def gen(i):
        for j in range(i):
            if j == 0:
                new_g = g()
            new_e = next(new_g)
            yield new_e             # This template is the stupidest one in the entire course.
    i = 0
    for _ in g():
        i += 1
        yield gen(i)
