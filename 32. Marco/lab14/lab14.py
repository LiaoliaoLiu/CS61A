...
def prune_min(t):
    ...
    if not t.is_leaf():
        b = t.branches
        t.branches = [b[0]] if b[0].label < b[1].label else [b[1]]
...
def num_splits(s, d):
    ...
    def helper(s, g1, g2):
        if s:
            return helper(s[1:], g1+[s[0]], g2) + helper(s[1:], g1, g2+[s[0]])
        else:
            if abs(sum(g1)-sum(g2)) <= d:
                return 1
            else:
                return 0
...
def align_skeleton(skeleton, code):
    ...
    def helper_align(skeleton_idx, code_idx):
        ...
        if skel_char == code_char:
            match = helper_align(skeleton_idx+1, code_idx+1)
            edits, cost = match
            possibilities.append((code_char + edits, cost))
        # Insert
        insert = helper_align(skeleton_idx, code_idx+1)
        edits, cost = insert
        possibilities.append(('+[{}]'.format(code_char) + edits, 1 + cost))
        # Delete
        delete = helper_align(skeleton_idx+1, code_idx)
        edits, cost = delete
        possibilities.append(('-[{}]'.format(skel_char) + edits, 1 + cost))
...
def filterl(lst, pred):
    ...
    #return reverse(foldl(lst, lambda rest, first: Link(first, rest) if pred(first) else rest, Link.empty))
...
def reverse(lst):
    ...
    if lst is Link.empty or lst.rest is Link.empty:
        return lst
    head = reverse(lst.rest)
    lst.rest.rest = lst         # FYI, question yourself why not use head.rest = lst
    lst.rest = Link.empty
    return head
