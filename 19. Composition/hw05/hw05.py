...
class Coin:
    ...
    def worth(self):
        ...
        return self.cents + (extra-50 if extra else 0)
...
def is_bst(t):
    ...
    min_v = -float('inf')
    max_v = float('inf')
    def helper(t, min_v, max_v):
        if t:
            if t.label < min_v or t.label > max_v or len(t.branches) > 2:
                return False
            else:
                if len(t.branches) == 1:
                    return helper(t.branches[0], min_v, t.label) or helper(t.branches[0], t.label, max_v)
                elif len(t.branches) == 2:
                    return helper(t.branches[0], min_v, t.label) and helper(t.branches[1], t.label, max_v)
                return True
        else:
            return True

    return helper(t, min_v, max_v)
